...
$('#testlesson').click(function(){
$('.testdiv').css('display','flex');
$('.warprtest').css('filter','blur(4px)');
});
var testcount=0;


$('#endtest2').click(function(){
 $('.warprtest').css('filter','none');
$('.testdiv').css('display','none');
$('#testlesson').hide();
alert('Кількість вірних відповідей ='+ testcount);

});

$('.trueanswert ').click(function(){
    testcount++;
    
    
});
$('.testbox button').click(function(){
    var testdiv=$(this).closest('div');
    testdiv.closest('div').css('transform','translateY(1000px)');
    setTimeout( function(){
     testdiv.closest('div').css('display','none');
    },800) ;
